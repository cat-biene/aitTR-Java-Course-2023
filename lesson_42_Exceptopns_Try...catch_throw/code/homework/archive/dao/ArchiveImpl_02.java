...
public class ArchiveImpl_02 implements Archive_02 {
    // этот компаратор позволяет сортировать объекты типа Document сначала по дате, потом по ID документа
    static Comparator<Document_02> comparator = (d1, d2) -> {
        int res = d1.getDate().compareTo(d2.getDate());
        return res != 0 ? res : Integer.compare(d1.getDocumentId(), d2.getDocumentId());
    };

    //fields
    private Document_02[] documents; // хранение документов в массиве
    private int size; // количество документов

    // constructor
    public ArchiveImpl_02(int capacity) {
        documents = new Document_02[capacity];
        // this.size = 0;
    }

    @Override
    public boolean addDocument(Document_02 document) {
    ...
    public boolean updateDocument(int folderId, int documentId, String url) {
        Document_02 document = getDocFromFolder(folderId, documentId);
        if (document == null) {
            return false;
        }
    ...
    @Override
    public Document_02 getDocFromFolder(int folderId, int documentId) {
        Document_02 pattern = new Document_02(folderId, documentId, null, null, null);
        for (int i = 0; i < size; i++) {
    ...
    @Override
    public Document_02[] getAllDocFromFolder(int folderId) {
    ...
    private Document_02[] findByPredicate(Predicate<Document_02> predicate) { // возвращаем массив найденных объектов
        Document_02[] res = new Document_02[size]; // объявили массив избыточной длины
    ...
    @Override
    public Document_02[] getDocBetweenDate(LocalDate dateFrom, LocalDate dateTo) {
        Document_02 pattern = new Document_02(0, Integer.MIN_VALUE, null, null, dateFrom.atStartOfDay()); // почему Integer.MIN_VALUE, а не 0
        ...
        pattern = new Document_02(0, Integer.MAX_VALUE, null, null, LocalDateTime.of(dateTo, LocalTime.MAX));// находим
        // правый край
