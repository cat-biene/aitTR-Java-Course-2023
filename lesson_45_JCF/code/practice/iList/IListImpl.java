...
public class IListImpl<E> implements IList1<E>{
    ...
    // иначе в списке может остаться ссылка на ненужный объект,
    // что может вызвать утечку памяти.
    //
    //Этот шаг необходим для правильной работы списка и предотвращения утечек памяти.
    // В Java объекты не удаляются сразу после вызова remove,
    // и сборка мусора может произойти позже.
    // Поэтому затирание последнего элемента является хорошей практикой
