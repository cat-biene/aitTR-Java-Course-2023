...
public class ArchiveImpl_01 implements Archive_01{
    // этот компаратор позволяет сортировать объекты типа Document сначала по дате, потом по ID документа
    static Comparator<Document_01> comparator = (d1, d2) -> {
        int res = d1.getDate().compareTo(d2.getDate());
        return res != 0 ? res : Integer.compare(d1.getDocumentId(), d2.getDocumentId());
    };

    //fields
    private Document_01[] documents; // хранение документов в массиве
    private int size; // количество документов

    // constructor
    public ArchiveImpl_01(int capacity) {
        documents = new Document_01[capacity];
        // this.size = 0;
    }

    @Override
    public boolean addDocument(Document_01 document) {
    ...
    public boolean updateDocument(int folderId, int documentId, String url) {
        Document_01 document = getDocFromFolder(folderId, documentId);
        if (document == null) {
            return false;
        }
    ...
    @Override
    public Document_01 getDocFromFolder(int folderId, int documentId) {
        Document_01 pattern = new Document_01(folderId, documentId, null, null, null);
        for (int i = 0; i < size; i++) {
    ...
    @Override
    public Document_01[] getAllDocFromFolder(int folderId) {
    ...
    private Document_01[] findByPredicate(Predicate<Document_01> predicate) { // возвращаем массив найденных объектов
        Document_01[] res = new Document_01[size]; // объявили массив избыточной длины
    ...
    // O(n)
    @Override
    public Document_01[] getDocBetweenDate(LocalDate dateFrom, LocalDate dateTo) {
        // O(1)
        Document_01 pattern = new Document_01(0, Integer.MIN_VALUE, null, null, dateFrom.atStartOfDay()); // почему Integer.MIN_VALUE, а не 0
        ...
        // O(1)
        pattern = new Document_01(0, Integer.MAX_VALUE, null, null, LocalDateTime.of(dateTo, LocalTime.MAX));// находим
        // правый край
        // O(log(n))
    ...
    @Override
    public Document_01[] getDocBetweenDateTwo(LocalDate dateFrom, LocalDate dateTo) {
        Document_01[] docArray = new Document_01[size];
        int j = 0;
        for (int i = 0; i < size; i++) {
